...
/****************************************************************************/
options metaserver=Example /* network name/address of the      */
                                        /*   metadata server.               */
        ...
        metapass="PASSWD"         /* Password for userid above.           */
...
%let ADServer = "example.com"; /* network name/address of   */
                                               /*   the AD server.          */
%let ADPort   = 389;                         /* Port on which LDAP interface*/
                                             /* is listening. 389 is a      */
                                             /* standard port.              */
%let ADPerBaseDN ="DC=example,DC=com,DC=br"; /* Specify the Distinguished */
                                             /* Name in the LDAP hierarchy  */
                                             /* where People searches begin.*/
%let ADGrpBaseDN ="DC=example,DC=com,DC=br"; /* Specify the Distinguished */
                                             /* Name in the LDAP hierarchy  */
                                             /* where Group searches begin. */
%let ADBindUser = "user@example.com";      /* Userid which will connect   */
...
/* Choices are the DistinguishedName of the User entry or the sAMAccountName.   */
/* For groups, the DistinguishedName will be used.                          */
/* %let keyidvar=sAMAccountName;                                                */
/* %let keyidvar=distinguishedName; sAMAccountName                          */
/****************************************************************************/
%let keyidvar=sAMAccountName;
%let MetadataAuthDomain=DefaultAuth;
%let WindowsDomain=;

LIBNAME AD BASE "example/example/AdirLib";
;
%let importlibref=AD;
...
%macro ldapextrpersons;
       ...
       attrs="displayName streetAddress cn company mail sAMAccountName " ||
       ...
       do eIndex = 1 to num;
          ...
          if (numAttrs > 0) then do aIndex = 1 to numAttrs;
             ...
             /* extract the mail - Email-Addresses (multi-valued) */
                ...
                mail= value;
             /* extract the sAMAccountName - Employee-ID */
             /*************************************************/
             /* sAMAccountName may need to be normalized/cleansed */
             /*************************************************/

             else if (attrName = 'sAMAccountName')  then  do;
                sAMAccountName= compress(value, "<>""");
             end;
          ...
          /* not use the sAMAccountName attribute, then this condition will need */ 
          ...
          /*       distinguishedName instead of sAMAccountName may not be useful.*/
...
data &extractlibref..ldapusers                                                               
     (keep= displayName streetAddress cn company mail sAMAccountName facsimileTelephoneNumber 
     ...
     length entryname $200 attrName $100 value $600 filter $1000
            displayName $256 streetAddress $100 cn $40 company $50 mail $50 
            sAMAccountName $40 facsimileTelephoneNumber $50 distinguishedName $200
            l $50 mobile $50 otherTelephone $50 physicalDeliveryOfficeName $50
            postalCode $20 name $60 sAMAccountName $40 st $20 telephoneNumber $50
     ...
     filter="(&(&(objectCategory=person)(objectClass=user))(memberOf=CN=EXAMPLE,OU=Example,DC=Example,DC=com,DC=br))";
     %ldapextrpersons
...
data &extractlibref..ldapgrps
     ...
     length entryname $200 attrName $100 value $600 filter $1000
            ...
            distinguishedName $200 sAMAccountName $40 member $200 
